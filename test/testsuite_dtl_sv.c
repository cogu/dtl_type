...
void test_dtl_sv_make(CuTest* tc)
{
	...
	CuAssertIntEquals(tc,124,dtl_sv_get_i32(sv));
	dtl_sv_delete(sv);
	...
	CuAssertIntEquals(tc,8328,dtl_sv_get_u32(sv));
	dtl_sv_delete(sv);
	...
	CuAssertTrue(tc,-1375713549903LL==dtl_sv_get_i64(sv));
	dtl_sv_delete(sv);
	...
	CuAssertTrue(tc,1375713549903ULL==dtl_sv_get_u64(sv));
	dtl_sv_delete(sv);
	...
	CuAssertDblEquals(tc,64.0,dtl_sv_get_flt(sv),0.001);
	dtl_sv_delete(sv);
	...
	CuAssertDblEquals(tc,83.0,dtl_sv_get_dbl(sv),0.001);
	dtl_sv_delete(sv);
	...
	CuAssertPtrEquals(tc,&i,dtl_sv_get_ptr(sv));
	dtl_sv_delete(sv);
	...
	CuAssertPtrEquals(tc,sv,dtl_sv_get_dv(sv2));
...
void test_dtl_sv_bool(CuTest* tc){
	...
	CuAssertIntEquals(tc,false,dtl_sv_get_bool(sv));
	dtl_ref_dec(sv);
	...
	CuAssertIntEquals(tc,DTL_SV_BOOL,dtl_sv_type(sv));
	CuAssertIntEquals(tc,true,dtl_sv_get_bool(sv));
	dtl_ref_dec(sv);
